...
export class EditPlanningComponent implements OnInit {
 ...
 async ngOnInit() {
    ...
    this.dateToday = moment(new Date()).add(-1, 'days').format('YYYY-MM-DD');
    this.PlanningForm = this.formBuilder.group({
      ...
      address_four: ['', [Validators.required]],
      ...
      this.planningService.updatePlanning(this.id, planning).subscribe( async (resp: any) => {
        if (!resp.success) {
          ...
          const resuls = await this.envoyerEmail(this.filterFormateur[0].email, this.Result.ref_formation,
            ...
            this.Result.type, this.Result.langue, planning.address_fournisseur, this.Result.version,
