...
export class ListExamenInactiveComponent implements OnInit {
  listOfData;
  listOfDisplayData;
  filterData = [];
  term = '';
  ...
  role;
  verifShowCategorie;
  constructor( private examenService: ExamenService,  private toastr: ToastrService, private authService: AuthService){  }
  ngOnInit(): void {
      this.fetchCours();
   ...
   fetchCours(): void{
    this.examenService.getAllCoursInactive().subscribe((data: any) => {
   ...
   CoursReturnActive(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.examenService.ReturnCoursActiveById(id).subscribe(
          async (resp: any) => {
             if (!resp.success) {
