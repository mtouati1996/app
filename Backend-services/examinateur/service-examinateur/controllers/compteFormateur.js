...
/*** start funtions for examens */
controler.getNumberExamen =  (req, res, next) => {
  try {
    const token = req.body.token;
    let decoded = jwt.verify(token, process.env.SECRET || SECRET );
      let id = decoded.results[0].id;
      ...
      let id3 = id;
      let id4 = id;
      let sql = "SELECT ( SELECT DISTINCT Utilisateurs.email FROM Utilisateurs WHERE Utilisateurs.id = ? ) AS nomformateur, (SELECT COUNT(Planning_Examinateur.id) FROM Planning_Examinateur INNER JOIN Utilisateurs ON Utilisateurs.id = Planning_Examinateur.id_examinateur WHERE Planning_Examinateur.id_examinateur = ? and Planning_Examinateur.statut = 'active' ) AS numbertraining, (SELECT COUNT(Planning_Examinateur.id) FROM Planning_Examinateur INNER JOIN Utilisateurs ON Utilisateurs.id = Planning_Examinateur.id_examinateur INNER JOIN Session_Examen ON Session_Examen.id = Planning_Examinateur.id_session INNER JOIN Examen ON Examen.id = Session_Examen.id_examen WHERE Planning_Examinateur.statut = 'active' and Planning_Examinateur.id_examinateur = ? and ADDTIME(Session_Examen.date_examen ,  Examen.duree ) >= CURRENT_TIMESTAMP() ) AS numbertrainingfeauture, (SELECT COUNT(Planning_Examinateur.id) FROM Planning_Examinateur INNER JOIN Utilisateurs ON Utilisateurs.id = Planning_Examinateur.id_examinateur INNER JOIN Session_Examen ON Session_Examen.id = Planning_Examinateur.id_session INNER JOIN Examen ON Examen.id = Session_Examen.id_examen WHERE Planning_Examinateur.statut = 'active' and Planning_Examinateur.id_examinateur = ? and ADDTIME(Session_Examen.date_examen ,  Examen.duree ) < CURRENT_TIMESTAMP()) AS numbertrainingpast FROM Utilisateurs GROUP BY nomformateur" ;
      let query = conn.query(sql, [id, id2, id3, id4], (err, result) => {
        if (err) {
          res.json({
            success: false,
...
controler.getNumberExamenByCategory =  (req, res, next) => {
  try {
    const token = req.body.token;
    const cat = req.body.cat;
    const cat2 = cat;
    let decoded = jwt.verify(token, process.env.SECRET || SECRET );
      ...
      let sql = "SELECT (SELECT DISTINCT Categories.nom_categorie FROM Categories WHERE Categories.statut = 'active' and Categories.nom_categorie = ? ) AS nomcategorie, (SELECT COUNT(Session_Examen.id) FROM Session_Examen INNER JOIN Examen ON Examen.id = Session_Examen.id_examen WHERE Session_Examen.statut = 'active' and Examen.categorie = ? and Session_Examen.affecter = ? ) AS numbertraining FROM Categories GROUP by nomcategorie ";
      let query = conn.query(sql, [cat, cat2, id],(err, result) => {
        if (err) {
          res.json({
            success: false,
...
controler.getAllExamenFeautureById =  (req, res, next) => {
  try {
    const token = req.body.token;
    let decoded = jwt.verify(token, process.env.SECRET || SECRET );
      let id = decoded.results[0].id;
      let sql = "SELECT Planning_Examinateur.id ,Examen.ref_form as ref_formation, Session_Examen.etat_session, Session_Examen.id AS id_session , Session_Examen.langue, Session_Examen.lieu, Examen.version, Session_Examen.date_examen, ADDTIME(Session_Examen.date_examen ,  Examen.duree ) AS date_fin  , CONCAT(Utilisateurs.nom,' ', Utilisateurs.prenom) AS name ,Planning_Examinateur.statut_f , Planning_Examinateur.nbr_participant FROM Planning_Examinateur LEFT JOIN Utilisateurs ON Utilisateurs.id = Planning_Examinateur.id_examinateur INNER JOIN Session_Examen ON Session_Examen.id = Planning_Examinateur.id_session INNER JOIN Examen ON Examen.id = Session_Examen.id_examen WHERE Planning_Examinateur.statut = 'active' and Planning_Examinateur.id_examinateur = ? and ADDTIME(Session_Examen.date_examen ,  Examen.duree ) >= CURRENT_TIMESTAMP()";
      let query = conn.query(sql, [id],(err, result) => {
        if (err) {
...
controler.getAllExamenPastById = async  (req, res, next) => {
  try {
    const token = req.body.token;
    let decoded = await jwt.verify(token, process.env.SECRET || SECRET );
      let id = decoded.results[0].id;
...
controler.getAllExamenforcalendarById =  (req, res, next) => {
  try {
    const token = req.body.token;
    let decoded = jwt.verify(token, process.env.SECRET || SECRET );
      let id = decoded.results[0].id;
      let sql = "SELECT Planning_Examinateur.id , CONCAT(Examen.ref_form ,' - ', IF( ISNULL(Session_Examen.lieu) , 'VT' , Session_Examen.lieu )) AS title , Session_Examen.date_examen as start, ADDTIME(Session_Examen.date_examen ,  Examen.duree ) as end FROM Planning_Examinateur LEFT JOIN Utilisateurs ON Utilisateurs.id = Planning_Examinateur.id_examinateur INNER JOIN Session_Examen ON Session_Examen.id = Planning_Examinateur.id_session INNER JOIN Examen ON Examen.id = Session_Examen.id_examen WHERE Planning_Examinateur.statut = 'active' and Planning_Examinateur.id_examinateur = ? ";
      let query = conn.query(sql, [id],(err, result) => {
        if (err) {
          res.json({
            success: false,
...
controler.getAllExamenById =  (req, res, next) => {
  try {
    const token = req.body.token;
    let decoded = jwt.verify(token, process.env.SECRET || SECRET );
      let id = decoded.results[0].id;
